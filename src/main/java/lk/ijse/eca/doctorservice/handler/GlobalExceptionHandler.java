...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(DoctorSlotNotFoundException.class)
    public ResponseEntity<ProblemDetail> handleProgramNotFound(
            DoctorSlotNotFoundException ex, HttpServletRequest request) {
        log.warn("DoctorSlot not found: {}", ex.getMessage());
        ProblemDetail problem = buildProblemDetail(
                HttpStatus.NOT_FOUND, "DoctorSlot Not Found", ex.getMessage(), request.getRequestURI());
        return problemResponse(HttpStatus.NOT_FOUND, problem);
    }

    @ExceptionHandler(DuplicateDoctorSlotException.class)
    public ResponseEntity<ProblemDetail> handleDuplicateProgram(
            DuplicateDoctorSlotException ex, HttpServletRequest request) {
        log.warn("Duplicate doctorSlot: {}", ex.getMessage());
        ProblemDetail problem = buildProblemDetail(
                HttpStatus.CONFLICT, "Duplicate DoctorSlot", ex.getMessage(), request.getRequestURI());
